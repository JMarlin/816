...
void EMUL_hardwareUpdate(word32 timestamp) {
	...
	if(!sending_file && _kbhit()) {

		input_register = _getch();
...
byte MEM_readMem(word32 address, word32 timestamp, word32 emulFlags) {
    ...
    if(address & 0x8000) {
		...
		if(sending_file) {
			...
			input_register = send_file_data[send_file_index++];
			uart_register = 0x01;

			if(send_file_index == send_file_size) {
		...
		b = input_register;
		input_register = 0;
		if(!sending_file) uart_register = 0;
